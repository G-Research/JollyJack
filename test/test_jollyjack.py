        ...
        for (n_rows, n_columns) in [(5,6), (1, 1), (100, 200), ]:
            for dtype in [pa.float16(), pa.float32(), pa.float64()]:
                with self.subTest((n_rows, n_columns, dtype)):
                    ...
                    jj.transpose_shuffle(src_array = src_array, dst_array = dst_array, row_indices = range(n_rows))
                    self.assertTrue(np.array_equal(expected_array, dst_array), f"{expected_array}\n!=\n{dst_array}")

                    # Reversed rows
                    jj.transpose_shuffle(src_array = src_array, dst_array = dst_array, row_indices = [n_rows - i - 1 for i in range(n_rows)])
                    ...
                    jj.transpose_shuffle(src_array = src_view, dst_array = dst_view, row_indices = range(n_rows - 2))
