...
supported_dtype_encodings = [
    ...
    (pa.float32(), "PLAIN"),
    (pa.float32(), "BYTE_STREAM_SPLIT"),
    (pa.float64(), "PLAIN"),
    (pa.float64(), "BYTE_STREAM_SPLIT"),
    (pa.int32(), "PLAIN"),
    (pa.int32(), "BYTE_STREAM_SPLIT"),
    (pa.int32(), "DELTA_BINARY_PACKED"),
    (pa.int64(), "PLAIN"),
    (pa.int64(), "BYTE_STREAM_SPLIT"),
    (pa.int64(), "DELTA_BINARY_PACKED"),
]
...
def get_table_with_nulls(n_rows, n_columns, data_type=pa.float32()):
    ...
    nullable_types = {
        pa.float32(): "Float32",
        pa.float16(): pa.float16().to_pandas_dtype(),
        pa.float64(): "Float64",
        pa.int16(): "Int16",
        pa.int32(): "Int32",
        pa.int64(): "Int64",
    ...
    for i in range(n_columns):
        if pa.types.is_integer(data_type):
            data[f"column_{i}"] = pd.array(
                np.random.randint(-100, 100, size=n_rows),
                dtype=nullable_types[data_type],
            )
        else:
            data[f"column_{i}"] = pd.array(
                np.random.uniform(-100, 100, size=n_rows),
                dtype=nullable_types[data_type],
            )
...
def get_table(n_rows, n_columns, data_type=pa.float32()):
    ...
    pa_arrays = [
        pa.array(data[:, i]).cast(data_type, safe=False) for i in range(n_columns)
    ]
...
class TestJollyJack(unittest.TestCase):
    ...
    def test_read_with_palletjack(self, pre_buffer, use_threads, use_memory_map):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for rg in range(n_row_groups):
                ...
                metadata = pj.read_metadata(
                    index_path, row_groups=[rg], column_indices=column_indices
                )
    ...
    def test_read_unsupported_column_types(
        self, pre_buffer, use_threads, use_memory_map
    ):

        with tempfile.TemporaryDirectory() as tmpdirname:
            path = os.path.join(tmpdirname, "my.parquet")
            table = get_table(
                n_rows=chunk_size, n_columns=n_columns, data_type=pa.bool_()
            )
            pq.write_table(
                table,
                path,
            ...
            self.assertTrue(
                f"has unsupported data type: 0!" in str(context.exception),
                context.exception,
            )
    ...
    def test_read_unsupported_encoding_dictionary(self, dtype):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            # Create an array of zerosx
            np_array = np.zeros(
                (chunk_size, n_columns), dtype=dtype.to_pandas_dtype(), order="F"
            )
            ...
            self.assertTrue(
                f"due to unsupported_encoding=RLE_DICTIONARY!"
    ...
    def test_read_unsupported_encoding_delta_byte_array(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            path = os.path.join(tmpdirname, "my.parquet")
            table = get_table(
                n_rows=chunk_size, n_columns=n_columns, data_type=pa.float16()
            )
            pq.write_table(
                table,
                path,
            ...
            # Create an array of zerosx
            np_array = np.zeros(
                (chunk_size, n_columns), dtype=pa.float16().to_pandas_dtype(), order="F"
            )
            ...
            self.assertTrue(
                f"due to unsupported_encoding=DELTA_BYTE_ARRAY!"
                in str(context.exception),
                context.exception,
            )
    ...
    def test_read_dtype_numpy(
        ...
        for n_row_groups, n_columns, chunk_size in [
            ...
            (
                ...
                1,
                10_000_001,
            ),  # +1 to make sure it is not a result of multip,lication of a round number
        ]:
            ...
            with self.subTest(
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    path = os.path.join(tmpdirname, "my.parquet")
                    table = get_table(
                        n_rows=n_rows, n_columns=n_columns, data_type=dtype
                    )
                    use_dictionary = False
                    ...
                    np_array = np.zeros(
                        (n_rows, n_columns), dtype=dtype.to_pandas_dtype(), order="F"
                    )
                    ...
                    self.assertTrue(
                        np.array_equal(np_array, expected_data),
                        f"{np_array}\n{expected_data}",
                    )
                    pr.close()
            ...
            with self.subTest((n_row_groups, n_columns, chunk_size, dtype, encoding)):
                ...
                with tempfile.TemporaryDirectory() as tmpdirname:
                    ...
                    tensor = torch.zeros(
                        n_columns,
                        n_rows,
                        dtype=numpy_to_torch_dtype_dict[dtype.to_pandas_dtype()],
                    ).transpose(0, 1)
                    ...
                    expected_data = pr.read_all().to_pandas().to_numpy()
                    self.assertTrue(
                        np.array_equal(tensor.numpy(), expected_data),
                        f"{tensor.numpy()}\n{expected_data}",
                    )
                    pr.close()
    ...
    def test_read_numpy_column_names(
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            self.assertTrue(
                np.array_equal(np_array, expected_data), f"{np_array}\n{expected_data}"
            )
            pr.close()
    ...
    def test_read_invalid_column(self, pre_buffer, use_threads, use_memory_map, dtype):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            self.assertTrue(
                f"Column 'foo_bar_0' was not found!" in str(context.exception),
                context.exception,
            )
            ...
            ss = [
                f"The file only has {n_columns} columns, requested metadata for column: {n_columns}",
                f"Trying to read column index {n_columns} but row group metadata has only {n_columns} columns",
            ]
            self.assertTrue(
                any(s in str(context.exception) for s in ss), context.exception
            )
    ...
    def test_read_invalid_row_group(
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            ss = [
                f"The file only has {n_row_groups} row groups, requested metadata for row group: {n_row_groups}",
    ...
    def test_read_not_enough_rows(self, pre_buffer, use_threads, use_memory_map, dtype):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            # Create an empty array
            np_array = np.zeros(
                (n_rows + 1, n_columns), dtype=dtype.to_pandas_dtype(), order="F"
            )
            ...
            self.assertTrue(
                f"Expected to read {n_rows + 1} rows, but read only {n_rows}!"
                in str(context.exception),
                context.exception,
            )
    ...
    def test_read_numpy_column_names_mapping(
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            jj.read_into_numpy(
                ...
                row_group_indices=range(n_row_groups),
                column_names={
                    f"column_{i}": n_columns - i - 1 for i in range(n_columns)
                },
                pre_buffer=pre_buffer,
                use_threads=use_threads,
                use_memory_map=use_memory_map,
            ...
            self.assertTrue(
                np.array_equal(np_array, reversed_expected_data),
    ...
    def test_read_not_enough_buffer(
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            self.assertTrue(
                f"Attempted to read {chunk_size} rows into location [0, {n_columns}], but that is beyond target's boundaries."
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Buffer overrun error: Attempted to read {chunk_size} rows into location [{chunk_size}, {n_columns - 1}], but there was space available for only {chunk_size - 1} rows."
                in str(context.exception),
                context.exception,
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with self.assertRaises(RuntimeError) as context:
                jj.read_into_numpy(
                    ...
                    use_memory_map=use_memory_map,
                    row_ranges=[
                        slice(0, 2 * chunk_size),
                    ],
                )
            self.assertTrue(
                f"Requested to read {2 * chunk_size} rows, but the current row group has only {chunk_size} rows"
                in str(context.exception),
                context.exception,
            )
            ...
            with self.assertRaises(RuntimeError) as context:
                jj.read_into_numpy(
                    ...
                    row_ranges=[
                        slice(0, chunk_size),
                        slice(chunk_size, 2 * chunk_size - 1),
                    ],
                )
            self.assertTrue(
                f"Requested to read {chunk_size - 1} rows, but the current row group has {chunk_size} rows"
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Row range 'slice(0, {chunk_size}, 2)' is not contiguous"
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Row range 'slice(None, {chunk_size}, None)' is not a valid range"
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Row range 'slice({chunk_size}, None, None)' is not a valid range"
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Row range 'slice({chunk_size}, {chunk_size - 1}, None)' is not a valid range"
                in str(context.exception),
                context.exception,
            )
            ...
            self.assertTrue(
                f"Expected to read 2 row ranges, but read only 1!"
                in str(context.exception),
                context.exception,
            )
    ...
    def test_copy_to_numpy_row_major(self, dtype, n_rows_n_columns):
        ...
        dst_array = np.zeros(
            (n_rows, n_columns), dtype=dtype.to_pandas_dtype(), order="C"
        )
        expected_array = np.zeros(
            (n_rows, n_columns), dtype=dtype.to_pandas_dtype(), order="C"
        )
        np.copyto(expected_array, src_array)
        jj.copy_to_numpy_row_major(
            src_array=src_array, dst_array=dst_array, row_indices=range(n_rows)
        )
        self.assertTrue(
            np.array_equal(expected_array, dst_array),
            f"{expected_array}\n!=\n{dst_array}",
        )
        ...
        jj.copy_to_numpy_row_major(
            src_array=src_array,
            dst_array=dst_array,
            row_indices=[n_rows - i - 1 for i in range(n_rows)],
        )
        ...
        jj.copy_to_numpy_row_major(
            src_array=src_view, dst_array=dst_view, row_indices=range(n_rows - 2)
        )
        self.assertTrue(
            np.array_equal(expected_array, dst_view),
            f"{expected_array}\n!=\n{dst_view}",
        )

        # Scattered rows
        dst_array = np.zeros(
            (2 * n_rows, n_columns), dtype=dtype.to_pandas_dtype(), order="C"
        )
        expected_array = np.zeros(
            (2 * n_rows, n_columns), dtype=dtype.to_pandas_dtype(), order="C"
        )
        ...
        jj.copy_to_numpy_row_major(
            src_array=src_array,
            dst_array=dst_array,
            row_indices=[i * 2 for i in range(n_rows)],
        )
        self.assertTrue(
            np.array_equal(expected_array, dst_array),
            f"{expected_array}\n!=\n{dst_array}",
    ...
    @parameterized.expand(
        itertools.product([pa.float16(), pa.float32(), pa.float64()], [1, 2], [1, 2])
    )
    def test_copy_to_torch_row_major(self, dtype, n_rows, n_columns):

        src_tensor = torch.tensor(
            get_table(n_rows, n_columns, data_type=dtype).to_pandas().to_numpy()
        )
        dst_tensor = torch.zeros(
            n_rows, n_columns, dtype=numpy_to_torch_dtype_dict[dtype.to_pandas_dtype()]
        )
        expected_tensor = src_tensor.clone().contiguous()
        jj.copy_to_torch_row_major(
            src_tensor=src_tensor, dst_tensor=dst_tensor, row_indices=range(n_rows)
        )
        self.assertTrue(
            torch.equal(expected_tensor, dst_tensor),
            f"{expected_tensor}\n!=\n{dst_tensor}",
        )
    ...
    @parameterized.expand(
        itertools.product([pa.float16(), pa.float32(), pa.float64()], [5, 50], [6, 60])
    )
    def test_copy_to_row_major_arg_validation(self, dtype, n_rows, n_columns):
        ...
        with self.assertRaises(AssertionError) as context:
            dst_array = np.zeros(
                (n_rows, n_columns + 1), dtype=dtype.to_pandas_dtype(), order="C"
            )
            jj.copy_to_numpy_row_major(
                src_array=src_array, dst_array=dst_array, row_indices=range(n_rows)
            )
        self.assertTrue(
            f"src_array.shape[1] != dst_array.shape[1], {n_columns} != {n_columns + 1}"
            in str(context.exception),
            context.exception,
        )
        ...
        with self.assertRaises(AssertionError) as context:
            dst_array = np.zeros(
                (n_rows - 1, n_columns), dtype=dtype.to_pandas_dtype(), order="C"
            )
            jj.copy_to_numpy_row_major(
                src_array=src_array, dst_array=dst_array, row_indices=range(n_rows)
            )
        self.assertTrue(
            f"src_array.shape[0] > dst_array.shape[0], {n_rows} > {n_rows - 1}"
            in str(context.exception),
            context.exception,
        )
        ...
        self.assertTrue(
            f"Expected destination array in a C (row-major) order"
            in str(context.exception),
            context.exception,
        )
        ...
        with self.assertRaises(AssertionError) as context:
            ...
            jj.copy_to_numpy_row_major(
                src_array=src_array, dst_array=dst_array, row_indices=range(n_rows - 1)
            )
        self.assertTrue(
            f"Unexpected len of row indices, {n_rows - 1} != {n_rows}"
            in str(context.exception),
            context.exception,
        )
        ...
        with self.assertRaises(AssertionError) as context:
            ...
            jj.copy_to_numpy_row_major(
                src_array=src_array,
                dst_array=dst_array,
                row_indices=[i - 1 for i in range(n_rows)],
            )
        self.assertTrue(
            f"Row index = -1 is not in the expected range [0, {n_rows})"
            in str(context.exception),
            context.exception,
        )
        ...
        self.assertTrue(
            f"Row index = {n_rows} is not in the expected range [0, {n_rows})"
            in str(context.exception),
            context.exception,
        )
