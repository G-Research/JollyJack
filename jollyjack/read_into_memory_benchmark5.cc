  ...
  arrow::io::CacheOptions cache_options)
{
  ...
  auto localFs = std::make_unique<arrow::fs::LocalFileSystem>();
  auto file_reader = localFs->OpenInputFile(path).ValueOrDie();
    ...
    auto result = ::arrow::internal::OptionalParallelFor(use_threads, read_ranges.size(),
            [&](int target_column) { 
              try
              {
                ...
                auto result = file_reader->ReadAt(read_range.offset, read_range.length, buffer->mutable_data()).ValueOrDie();
